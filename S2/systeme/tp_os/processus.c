...
int gestion_interruption( int num_processus_courant ){
	...
	if( inter_a_traiter->id_origine == num_processus_courant ){
		...
		// *************************FIN_APPEL_BLOQUNT *******************************
			...
			// interuption : fin appel bloquant
...
int gestion_instruction_2( struct liste_processus *listeProc, int num_processus ){
	...
	// on va le chercher et executer son instruction
	...
	while( parcourProcessus != NULL ){
		...
		if( parcourProcessus->num_processus == num_processus ){
			...
			// il est en execution maintenant alors je change son etat
			parcourProcessus->etat = 0;
			// je l'ai trouvé alors j'execute son instruction
			// je regarce si l'instruction est valide 
			
			int indice_instruction = parcourProcessus->pt_pile;
			int valide = parcourProcessus->programme[ indice_instruction ].valide;
			...
			if( valide == 0){
				...
				genere_interruption_2( parcourProcessus->num_processus, -2 , 0 );
				// je change son etat maintenant à en attente
				parcourProcessus->etat = 1;
				//parcourProcessus->pt_pile++;  je ne sais pas si je le fais ou pas
				...
				int bloquant_processus = parcourProcessus->programme[ indice_instruction ].bloquant_processus ;
				int bloquant_ressource = parcourProcessus->programme[ indice_instruction ].bloquant_ressource ;
				...
				if(  bloquant_processus == 0 && bloquant_ressource == 0){
					...
					printf("instruction valide de processus num : %d\n", parcourProcessus->num_processus );
...
void ordonnancement_2(int nb_tours ){
	...
	int nb_instr = 7;   // c'est le nombre d'instruction à executer après chaque tour
	int i , j, k;
	// je commence par la liste des processus pret à s'execute et je n'execute pas le init
	struct processus *parcourProcessus = liste_processus->premier->next;
	
	for( i = 0; i < nb_tours; i++ ){
		
		// j'execute ses instructions	
	  	while( parcourProcessus != NULL ){
			printf("parcourProcessus->num_processus : %d\n", parcourProcessus->num_processus );	
			// pour chaque tour j'execute 3 instructions
			
			nb_instr = 3;	
			
			for( j = 0; j < nb_instr; j++ ){
				
				k = parcourProcessus->pt_pile;
				
				if( parcourProcessus->programme[ k ].valide == 1 ){
				
				
					// ici l'instruction est valide
					// je regarde si elle est bloquante
					
					if( parcourProcessus->programme[ k ].bloquante == 1 ){
						 					
						// oui elle est bloquante
						// j'ajoute ce processus dans les processus en attente
						// avant de l'ajouter il faut incrementer le pt_pile

						parcourProcessus->pt_pile++;
						
						// il faut que je regarde si le processus a fini
						if( parcourProcessus->pt_pile > parcourProcessus->nb_instruction){
						
							// si c'est la fin alors je le tue
							// j'adopte ses fils dans la fonction supprimer_proc
							supprimer_proc( liste_processus , parcourProcessus->num_processus );

							//  je sors de la boucle for car le processus a fini ses instructions
							break ;
						}
						
						// s'il a pas encore fini
						// je l'ajoute dans la liste des processus en attente
						// et je le supprime de la liste des processus pret à s'executer mais sans adopter 
						// ses fils
 								
						ajoute_en_fin( liste_processus , parcourProcessus );
						supprimer_proc( liste_processus , parcourProcessus->num_processus );

						break;
					}else{
					
						// non elle n'est pas bloquante
						
						
						parcourProcessus->pt_pile++;
						//  je regarde si le processus a fini
						if( parcourProcessus->pt_pile > parcourProcessus->nb_instruction){
						
							// si c'est la fin alors je le tue
							// j'adopte ses fils dans la fonction supprimer_proc_adopte_fils 
							supprimer_proc_adopte_fils( liste_processus , parcourProcessus->num_processus );
														    		    		    	
							//  je sors de la boucle for car le processus a fini ses instructions
							break ;
						}
						
						// si c'est pas encore la fin je continue avec les autres instructions
					...
					// je le supprime et j'adopte ses fils dans la fonction supprimer_proc_adopte_fils 

					supprimer_proc_adopte_fils( liste_processus , parcourProcessus->num_processus );

					//  je sors de la boucle for car le processus est  interrompu
					break ;
...
int ordonnancement(){
	...
	while( compteur !=  nb_cycle_max){
		
		// je prend le premier processus/
		struct processus *processus_en_cours = premier_proc_pret_executer();
	
		printf("\nle processus en execution est %d\n", processus_en_cours->num_processus );
		
		// je change son etat mnt 
		processus_en_cours->etat = 0;
		
		if( processus_en_cours == NULL ){
		...
		for( i = 0; i < tmp_cpu_processus ; i++){
			...
			gestion_interruption( processus_en_cours->num_processus );
			
			// j'execute une instruction de ce processus en cours
			ret = gestion_instruction( liste_processus , processus_en_cours );
			if( ret == 2 ){
